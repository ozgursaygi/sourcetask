...
namespace Dcm.Controllers
{
    public class GeneralController : Controller
    {
        ...
        public ActionResult ListPage(string MenuId)
        {
            ...
            dsList = GetDataList( Convert.ToInt32(MenuId), searchText, iDisplayStart, iDisplayLength, sortColumnIndex, sortColumnDir, ref totalCount, ref displayCount, ref detailPageName);
        ...
        public ActionResult AjaxHandler(JQueryDataTableParamModel param,int MenuId)
        {
            ...
            foreach (JObject o in arrayList.Children<JObject>())
            {
                ...
                foreach (JProperty p in o.Properties())
                {
                    ...
                    if (i == 1 && detailPageName.Trim()!="")
                    {
                        string url = Url.Content(detailPageName + "?MenuId=" + Dcm.Source.GlobalHelper.Encrypt(MenuId.ToString()) + "&RecordId=" + value );
                        stringList.Add("<a style='padding-left: 15px;'  href=" + url + " ><span class='fa fa-edit' style='font-size:18px;margin-top:12%'></span></a>");
                    }
                    else if(i>1)
                    {
        ...
        public DataSet GetDataList(int MenuId, string searchText, int iDisplayStart, int iDisplayLength,int sortColumnIndex,string sortColumnDir, ref string totalCount, ref string displayCount, ref string detailPageName)
        {
            ...
            if (dsMenu.Tables.Count > 0 && dsMenu.Tables[0].Rows.Count > 0)
            {
                ...
                ViewBag.NewRecordButton = dsMenu.Tables[0].Rows[0]["list_new_record_button"].ToString();
                ...
                string filter = (dsMenu.Tables[0].Rows[0]["filter"] != System.DBNull.Value && dsMenu.Tables[0].Rows[0]["filter"].ToString().Trim() !="")? "  " + dsMenu.Tables[0].Rows[0]["filter"].ToString() + (searchFilter.Trim()!=""?" and "+searchFilter:" ") : (searchFilter.Trim() != "" ? " " + searchFilter : " ");

                filter = BaseClasses.BaseFunctions.getInstance().ReplaceMenuFilterVariables(filter);

                dsList = BaseDB.DBManager.AppConnection.GetDataSet("select * from (select ROW_NUMBER() OVER (" + order_by + ") AS intRow , " + columns + " from " + object_name + (filter.Trim() != "" ? " where " + filter : "") + " ) as table1 " + selectStringByRowNumber + order_by);

                if (searchText != "" || filter!="")
                {
                    dsListWithSearchText = BaseDB.DBManager.AppConnection.GetDataSet("select * from (select ROW_NUMBER() OVER (" + order_by + ") AS intRow , " + columns + " from " + object_name + (filter.Trim() != "" ? " where " + filter : "") + " ) as table1 where 1=1 " + order_by);
                    displayCountWithSearchText =  dsListWithSearchText.Tables[0].Rows.Count.ToString();
                }

                DataSet dsTotalCount = BaseDB.DBManager.AppConnection.GetDataSet("select count(*) from " + object_name + (filter.Trim() != "" ? " where " + filter.Trim() : ""));
