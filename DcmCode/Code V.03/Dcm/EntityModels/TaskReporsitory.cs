...
namespace Gunluk.EntityModels
{
    public class TaskRepository : BaseDB.BaseRepository<Dcm.EntityModels.TaskEntities>
    {
        ...
        public DataSet GetAllClosedTaskByMonthChartData()
        {
            return BaseDB.DBManager.AppConnection.GetDataSet("select * from tsk_all_task_closed_by_month_chart_v");
        }
        ...
        public List<tsk_tasks_v> GetTaskList(string userId, DateTime startDate, DateTime endDate)
        {
            ...
            List<tsk_tasks_v>  result=null;
            if(!string.IsNullOrEmpty(userId) && userId!="0")
                result = db.tsk_tasks_v.Where(t => t.assigned_user_id == user && t.start_date1 >= startDate && t.start_date1 <= endDate ).OrderBy(x => x.order_id).ToList();
            else if (userId=="0")
                result = db.tsk_tasks_v.Where(t => t.start_date1 >= startDate && t.start_date1 <= endDate).OrderBy(x => x.order_id).ToList();
            return result;
        }

        public List<tsk_tasks_v> GetTaskListByStatusId(int statusId, DateTime startDate, DateTime endDate, string userId)
        {
            Guid user = Guid.Empty;
            ...
            List<tsk_tasks_v> result = null;
            if (!string.IsNullOrEmpty(userId) && userId != "0")
                result = db.tsk_tasks_v.Where(t => t.task_status_id == statusId && t.start_date1 >= startDate && t.start_date1 <= endDate && t.assigned_user_id == user).OrderBy(x => x.order_id).ToList();
            else if (userId == "0")
                result = db.tsk_tasks_v.Where(t => t.task_status_id == statusId && t.start_date1 >= startDate && t.start_date1 <= endDate).OrderBy(x => x.order_id).ToList();
        ...
        #region Görev Bilgilendirme İşlemleri

        public DataSet SendTaskExpireNotification()
        {
            return BaseDB.DBManager.AppConnection.GetDataSet("select * from tsk_expire_end_date_v");
        }
        #endregion
